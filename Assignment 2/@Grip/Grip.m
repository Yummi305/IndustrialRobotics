classdef Grip < RobotBaseClass
    %% Create Grip finger Robot

    properties(Access = public)              
        plyFileNameStem = 'Grip';
    end
    
    methods
%% Define robot Function 
function self = Grip(baseTr)
			self.CreateModel();
            if nargin < 1			
				baseTr = eye(4);				
            end
...
%% Create the robot model
        function CreateModel(self)   
            ...
            link(3) = Link('d',0,'a',-0.05,'alpha',pi, 'qlim', [deg2rad(-160) deg2rad(90)],'offset',0);% Revolute catch grip
